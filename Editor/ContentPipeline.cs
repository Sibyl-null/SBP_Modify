...
namespace UnityEditor.Build.Pipeline
{
    ...
    public static class ContentPipeline
    {
            ...
            out IBundleBuildResults result, IList<IBuildTask> taskList, params IContextObject[] contextObjects)
        {
            ...
            // 避免在这里抛出异常，因为我们不希望它们冒泡到调用用户代码
            if (parameters == null)
            {
                result = null;
                BuildLogger.LogException(new ArgumentNullException(nameof(parameters)));
                return ReturnCode.Exception;
            }
            ...
            {
                result = null;
                BuildLogger.LogException(new ArgumentException("Argument cannot be null or empty.", nameof(taskList)));
                return ReturnCode.Exception;
            }

            // 如果有未保存的场景更改，不要运行
            // 检查当前打开的场景，不是 BuildSettings 中的场景列表
            ...
            if (!buildContext.TryGetContextObject<IBuildLogger>(out var logger))
            {
                ...
                using (var buildCache = new BuildCache(parameters.CacheServerHost, parameters.CacheServerPort))
                {
                    ...
                    try
                    {
                        ...
                        if (!buildContext.ContainsContextObject(typeof(IDeterministicIdentifiers)))
                            buildContext.SetContextObject(parameters.ContiguousBundles
